...
###umap-learn need to be installed beforehand, if that doesn't work just remove ,umap="umap-learn"
Seurat_objects$postQC<-seurat_flow(Seurat_objects$postQC,dim=1:21,sct=T,norm=F,umap="umap-learn") 
...
Seurat_objects$processed<-seurat_flow(Seurat_objects$processed,res=0.5,dim = 1:21,umap="umap-learn",only.var = F)
...
cluster_abs<-graph_list_g$Seurat_cluster.col+geom_text(aes(label =paste(Freq,"cells",sep=" ")),  position = position_stack(vjust = 0.5),size=5)+geom_text(data=props_total_t,aes(label =total,y=prop,x=Genotype),   vjust = -.25,size=6)

# saveRDS(cluster_abs,here::here("figures","cluster_abs.RDS"))
plot_grid(DimPlot(Seurat_objects$processed,group.by = "Genotype"),DimPlot(Seurat_objects$processed,cols = my_cols2,label=T)+NoLegend(),cluster_abs)
...
ontology$nogeno["GO"]<-scRNAseq.analysis::GSEA(object = Seurat_objects$processed,condition = "Genotype",top = 50,GeneSets = "GO",total = T,OrgDb = OrgDb,human_ensembl = human_ensembl,mouse_ensembl = mouse_ensembl)
```
...
ontology_plots[["total"]]<-foreach(i=names(ontology$total),.packages = c("clusterProfiler"),.combine=c,.multicombine=T)%dopar%{
  ...
  ontology_plots[[i]]<-clusterProfiler::dotplot(ontology$total[[i]],x=~cluster,show=5)+theme(axis.text.x = element_text(angle=60,vjust = 1, hjust=1,size=13),axis.text.y = element_text(size=12))+ggtitle(i)
...
ontology_plots[["cluster"]]<-foreach(i=names(ontology$cluster),.packages = c("clusterProfiler"),.combine=c,.multicombine=T)%dopar%{
 ...
 plots[[i]]<-dotplot(ontology$cluster[[i]],x=~cluster,show=5)+facet_wrap(~Condition)+theme(axis.text.x = element_text(angle=60,vjust = 1, hjust=1,size=13),axis.text.y = element_text(size=12))+ggtitle(i)
...
ontology_plots[["nogeno"]]<-foreach(i=names(ontology$nogeno),.packages = c("clusterProfiler"),.combine=c,.multicombine=T)%dopar%{
 ...
 plots[[i]]<-dotplot(ontology$nogeno[[i]],x=~cluster,show=5)+theme(axis.text.x = element_text(angle=60,vjust = 1, hjust=1,size=13),axis.text.y = element_text(size=12))+ggtitle(i)
...
plot_grid(plotlist = ontology_plots$total[c("KEGG","DO")])
...
for (i in levels(Idents(tmp))){
  ...
  dataset.markers$cluster[[i]] <-FindMarkers(object = tmp2,ident.1 = "KO",ident.2 = "WT")
...
dataset.markers$cluster$ss_MG$cluster
Volcano_plots<-llply(dataset.markers$cluster,Volcano_plot,do.col=F,Condition="cluster",FC_cutoff=0.25,p_cutoff=1e-02,legend="none",.parallel = T,.paropts = list(.packages = "EnhancedVolcano"))

Volcano_plots$cycling<-Volcano_plot(dataset.markers$cluster$cycling,FC_cutoff=0.25,p_cutoff=1e-03,legend="right")

Volcano_plots$CAM<-Volcano_plot(dataset.markers$cluster$CAM,FC_cutoff=0.25,p_cutoff=1e-02,legend="none")

Volcano_plots$DAM_MG<-Volcano_plot(dataset.markers$cluster$DAM_MG,FC_cutoff=0.25,p_cutoff=1e-02,legend="none")

Volcano_plots$MG<-Volcano_plot(dataset.markers$cluster$MG,FC_cutoff=0.25,p_cutoff=1e-02,legend="none")
names(Volcano_plots)<-names(dataset.markers$cluster)

Volcano_plots<-Volcano_plots[-c("inflam_DAM","type_1_MF","inflam_NP")]

plot_grid(plotlist = Volcano_plots)
##visualise all at once 
  table_cluster<-table(dataset@active.ident,dataset$Genotype)
  plot_grid(plotlist=Volcano_plots$nosoup[1],labels=names(Volcano_plots$nosoup)[1])
  plot_grid(DimPlot(dataset,split.by="Genotype"),DimPlot(dataset))
  
  dataset<-Seurat_objects$processed
  
  cluster_marker_genes<-list()
