...
#' @export
abs_by_cond<-function(seuratobject,condition="Genotype",use_cols=NULL,font.total=3,font.stack=1.5,ctrl.group="WT",label.stack=T,label.total=T,title="Cluster Abundances")
{###extract frequencies of each seurat clusters splitted into conditions as table from Seurat object total
  ...
  cluster_abs<-graph_list_g$Seurat_cluster.col+
    ggtitle(title)+
    {if(label.stack==T)geom_text(aes(label =paste(Freq,"cells",sep=" ")),  position = position_stack(vjust = 0.5),size=font.stack)}+
    {if(label.total==T)geom_text(data=props_total_t,aes_string(label ="total",y="prop",x=condition),   vjust = -.25,size=font.total)}
  return(cluster_abs)
}
